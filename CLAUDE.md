...
1. **Nominate** — anyone can nominate a candidate via the form in [components/NominationForm.tsx](components/NominationForm.tsx) (rendered inside [components/NominationsSection.tsx](components/NominationsSection.tsx)). They submit `nominee_name` + `reasoning` (the public-facing case for the nominee), an optional photo, plus their own `submitter_name` + `submitter_email` and an opt-in `raffle_opt_in` checkbox (default off) for a >1000kr prize draw. The form posts `multipart/form-data` (not JSON) so the photo file can ride along. `app/api/nominate/route.ts` validates the photo (image mime type, ≤5MB), uploads it to the public `nomination-photos` Supabase Storage bucket, and inserts the row as `status: "unverified"` with a `verification_token`, then emails a confirmation link (`type=nomination`) to the submitter via [lib/email.ts](lib/email.ts) (Resend; silently no-ops if `RESEND_API_KEY` is unset).
2. **Vote** — voting also requires a confirmed email, the same shape as nominating: clicking "Stem" opens [components/VoteForm.tsx](components/VoteForm.tsx) inline (name + email), and `app/api/vote/route.ts` inserts a `votes` row as `status: "unverified"` with its own `verification_token`, emailing a confirmation link (`type=vote`). One vote per person *per nominee* — enforced by a unique index on `(nomination_id, lower(voter_email))`, not a global one, so the same person can back multiple different nominees. Race-safe (handles Postgres error code `23505`).
3. **Verify** — both flows share `app/api/verify/route.ts`, dispatched on a `type` query param (`nomination` | `vote`). A nomination flips to `status: "approved"` (visible + votable) and emails the admin; a vote flips to `status: "confirmed"` and starts counting. No token = no effect — this is the anti-spam/anti-ballot-stuffing gate, there's no separate manual approval step. [app/verify-result/page.tsx](app/verify-result/page.tsx) shows a confirmation page with copy that branches on `type`.
4. **Display** — [components/NominationsSection.tsx](components/NominationsSection.tsx) reads the `nomination_results` view (anon key, client-side), ranked by vote count. Results (percentages/counts) are visible to **everyone immediately**, not gated behind "have you voted" — vote confirmation is asynchronous (a link clicked later, possibly on a different device), so there's no reliable browser-side state to hide results behind. The view deliberately does not expose `submitter_name`/`submitter_email` or voter identities — those are admin-only.
5. **Moderation** — `app/admin/page.tsx` is a password-gated SPA listing every nomination (any status) via `app/api/admin/nominations` (service role, reads the unfiltered `nomination_admin_view`), with submitter details, raffle flag, and confirmed vote count. Admin can approve/reject (`PATCH` status) or permanently delete (`DELETE`, cascades votes) — same shared-secret auth model as CBS Industry Hub: `x-admin-password` header checked against `ADMIN_PASSWORD` env var (no hardcoded fallback — fails closed if unset), kept in component state and resent with every request, no per-user auth or session.
