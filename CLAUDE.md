...
[supabase/schema.sql](supabase/schema.sql) defines `nominations`, `votes`, and two views: `nomination_results` (public — approved only, no submitter PII) and `nomination_admin_view` (unfiltered, includes submitter details — only reachable via the service role key, never granted to anon/authenticated). RLS is enabled on both base tables with **no** public policies on `nominations` itself (it holds `submitter_email`); all writes go through the service-role API routes.
